...
public class Catalog {
    ...
    public StringBuilder getWholeCatalog() {
        ...
        for(int i = 0 ; i<items.size();i++){
            a += items.get(i)+"-"+prices.get(i)+ "-" + monthlyPayments.get(i)+ "\n";
        }
    ...
    public ArrayList<String> getItemsLessThanAMonthlyPrice(double price) {
        ...
        loadWholeCatalog();
        //TODO
        ArrayList <String> bobby = new ArrayList<>();

        for (int i = 0; i < items.size(); i++) {
            if(monthlyPayments.get(i)<= price){
                String a = items.get(i) + "-"+ prices.get(i) + "-" +monthlyPayments.get(i);
                bobby.add(a);
            }
        }
    ...
    public void deleteItemFromCatalog(String item) {
        ...
        for(int i =0 ; i<items.size();i++){
            if(items.get(i).equalsIgnoreCase(item)){
                items.remove(i);prices.remove(i);monthlyPayments.remove(i);
            }
        }
    }
